...
def check_reaction_ends_by_smiles(
    ...
    """
    # Sort copies rather than mutating the caller's lists in place.
    true_rc_smi_list = sorted(true_rc_smi_list)
    true_pc_smi_list = sorted(true_pc_smi_list)
    pred_rc_smi_list = sorted(pred_rc_smi_list)
    pred_pc_smi_list = sorted(pred_pc_smi_list)

    if (
        (true_rc_smi_list == pred_rc_smi_list and true_pc_smi_list == pred_pc_smi_list)
        or (true_rc_smi_list == pred_pc_smi_list and true_pc_smi_list == pred_rc_smi_list)
        or check_product_connectivity(
            true_rc_smi_list, true_pc_smi_list, pred_rc_smi_list, pred_pc_smi_list
        )
    ):
        return True
    else:
...
def check_reactant_product_graphs_identical(
    ...
    if (
        np.sum(np.abs(true_rc_adj_mat - pred_rc_adj_mat)) == 0
        and np.sum(np.abs(true_pc_adj_mat - pred_pc_adj_mat)) == 0
    ) or (
        np.sum(np.abs(true_rc_adj_mat - pred_pc_adj_mat)) == 0
        and np.sum(np.abs(true_pc_adj_mat - pred_rc_adj_mat)) == 0
    ):
        return True
    else:
        return False
...
def check_reactant_product_graphs_threshold(
    ...
    if (
        (
            np.sum(np.abs(true_rc_adj_mat - pred_rc_adj_mat)) == 0
            and np.sum(np.abs(true_pc_adj_mat - pred_pc_adj_mat)) <= irc_end_graph_threshold
        )
        or (
            np.sum(np.abs(true_rc_adj_mat - pred_rc_adj_mat)) <= irc_end_graph_threshold
            and np.sum(np.abs(true_pc_adj_mat - pred_pc_adj_mat)) == 0
        )
        or (
            np.sum(np.abs(true_rc_adj_mat - pred_pc_adj_mat)) == 0
            and np.sum(np.abs(true_pc_adj_mat - pred_rc_adj_mat)) <= irc_end_graph_threshold
        )
        or (
            np.sum(np.abs(true_rc_adj_mat - pred_pc_adj_mat)) <= irc_end_graph_threshold
            and np.sum(np.abs(true_pc_adj_mat - pred_rc_adj_mat)) == 0
        )
    ):
        return True
...
def check_reaction_ends_by_graph_topology(
    ...
    if irc_end_graph_threshold == 0:
        return check_reactant_product_graphs_identical(
            true_rc_adj_mat, true_pc_adj_mat, pred_rc_adj_mat, pred_pc_adj_mat
        )
    elif irc_end_graph_threshold > 0:
        return check_reactant_product_graphs_identical(
            true_rc_adj_mat, true_pc_adj_mat, pred_rc_adj_mat, pred_pc_adj_mat
        ) or check_reactant_product_graphs_threshold(
            true_rc_adj_mat,
            true_pc_adj_mat,
            pred_rc_adj_mat,
            pred_pc_adj_mat,
