...
def select_promising_reactant_product_pairs(
    ...
    if len(scores) == 1:
        print("Only 1 reactant & product complex was found")
        opt_idxs = [indices[0]]
        ...
        opt_idxs = indices[
            np.argpartition(scores, n_reactant_product_pairs)[:n_reactant_product_pairs]
        ]
    else:
...
def rmsd_score(rc_coords: np.ndarray, pc_coords: np.ndarray, align_complexes: bool = True) -> float:
    """
...
def weighted_rmsd_score(
    rc_coords: np.ndarray, pc_coords: np.ndarray, weights: np.ndarray, align_complexes: bool = True
) -> float:
    """
    ...
    if align_complexes:
        ...
        rc_coords_aligned = rc_coords
    score = np.sqrt(
        np.sum(weights * np.linalg.norm((pc_coords - rc_coords_aligned) ** 2, axis=-1))
        / np.sum(weights)
    )
    return score
...
def active_bond_distance_score(
    ...
    score = sum(
        [
            np.sqrt(np.mean((rc_coords_aligned[bond[0], :] - rc_coords_aligned[bond[1], :]) ** 2))
            for bond in bonds
        ]
    )
    return score
...
def compute_optimal_coordinates(
    rc_coordinates: np.ndarray, pc_coordinates: np.ndarray
) -> np.ndarray:
