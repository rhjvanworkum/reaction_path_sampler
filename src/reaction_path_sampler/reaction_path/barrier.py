...
def compute_barrier(
    ...
    if len(energies) > 0:
        ...
        energies = [
            xtb_single_point_method(reactant, charge, mult, solvent, n_cores=2)
    ...
    if settings["sample_ts_conformers"]:
        ts_conf_sampler = MetadynConformerSampler(
            smiles_strings=[], solvent=solvent, settings=settings
        )
        ts_confs = ts_conf_sampler.sample_ts_conformers(
            mol=MolecularSystem(
                smiles=None, rdkit_mol=None, geometry="".join(ts_geometry), charge=charge, mult=mult
            ),
