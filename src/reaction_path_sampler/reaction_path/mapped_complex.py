...
def correct_atom_map_numbers(reactants: list[Chem.Mol], products: list[Chem.Mol]) -> None:
    """
...
def get_autode_geometry_from_mapped_rdkit_conformer(mols: list[Chem.Mol]) -> list[Atom]:
    ...
    if len(mols) == 1:
        ...
        mols_atoms[0] = [
            [
                atom[0],
                atom[1]
                + min_dist * ((centroid_2 - centroid_1) / np.linalg.norm(centroid_2 - centroid_1)),
                atom[2],
                atom[3],
            ]
            for atom in mols_atoms[0]
        ]
        ...
        mols_atoms[1] = [
            ...
            for atom in mols_atoms[1]
        ]
        mols_atoms[2] = [
            [
                atom[0],
                atom[1]
                + min_dist * ((centroid_3 - centroid_1) / np.linalg.norm(centroid_3 - centroid_1)),
                atom[2],
                atom[3],
            ]
            for atom in mols_atoms[2]
        ]
    ...
    atoms = [
        Atom(atomic_symbol=atom[0], x=atom[1][0], y=atom[1][1], z=atom[1][2]) for atom in atoms
    ]
    return atoms, indices, species_complex_mapping


def generate_mapped_reaction_complexes(
    reactant_smiles: list[str], product_smiles: list[str], solvent: str
) -> list[Complex]:
    """
    ...
    reactants, products = (
        ...
        [Chem.MolFromSmiles(p) for p in products],
    )
    ...
    reactant_atoms, reactant_indices, species_complex_mapping = (
        get_autode_geometry_from_mapped_rdkit_conformer(reactants)
    )
    ...
    nx.set_node_attributes(
        rc_complex.graph, {i: v for i, v in enumerate(reactant_indices)}, "atom_index"
    )
    rc_complex.conformers = [
        Conformer(
            name=str(time.time()),
            atoms=reactant_atoms,
            charge=_rc_complex.charge,
    ...
    product_atoms, product_indices, species_complex_mapping = (
        get_autode_geometry_from_mapped_rdkit_conformer(products)
    )
    ...
    nx.set_node_attributes(
        pc_complex.graph, {i: v for i, v in enumerate(product_indices)}, "atom_index"
    )
    pc_complex.conformers = [
        Conformer(
            name=str(time.time()),
            atoms=product_atoms,
            charge=_pc_complex.charge,
