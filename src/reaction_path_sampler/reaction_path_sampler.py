...
from reaction_path_sampler.conformational_sampling.sample_conformers import (
    sample_reactant_and_product_conformers,
)
from reaction_path_sampler.graphs.xyz2mol import get_canonical_smiles_from_xyz_string
from reaction_path_sampler.interfaces.methods import barrier_calculation_methods_dict
...
from reaction_path_sampler.molecular_system import Reaction
from reaction_path_sampler.reaction_path.barrier import compute_barrier
from reaction_path_sampler.reaction_path.complexes import (
    compute_optimal_coordinates,
    select_promising_reactant_product_pairs,
)
from reaction_path_sampler.reaction_path.path_interpolation import interpolate_geodesic
...
from reaction_path_sampler.ts_template import get_constraints_from_template, save_ts_template
from reaction_path_sampler.utils import get_canonical_smiles, write_output_file


class ReactionPathSampler(ReactionSampler):
    def __init__(self, settings: dict[str, Any], reaction: Reaction) -> None:
    ...
    def sample_reaction_complex_conformers(
        ...
        rc_conformers, pc_conformers = sample_reactant_and_product_conformers(
            self.reaction.reactants, self.reaction.products, settings=self.settings
        )
        return rc_conformers, pc_conformers

    def select_promising_reactant_product_pairs(
        self, rc_conformers, pc_conformers, charge
    ) -> list[list[Conformer]]:
        """
    ...
    def _align_conformers(
        self, rc_conformer: Conformer, pc_conformer: Conformer, output_dir: str
    ) -> None:
        """
        Align the reactant and product conformers to each other.
        """
        rc_conformer._coordinates = compute_optimal_coordinates(
            rc_conformer.coordinates, pc_conformer.coordinates
        )
    ...
    def _create_interpolated_path(
        ...
        write_geodesic_xyz(
            os.path.join(output_dir, "geodesic_path.trj"), rc_conformer.atomic_symbols, curve.path
        )
        write_geodesic_xyz(
            os.path.join(output_dir, "geodesic_path.xyz"), rc_conformer.atomic_symbols, curve.path
        )
    ...
    def _perform_cos_and_tsopt(
        ...
        if tsopt is not None and imaginary_freq is not None:
            ...
            if (
                imaginary_freq < self.settings["min_ts_imaginary_freq"]
                and imaginary_freq > self.settings["max_ts_imaginary_freq"]
            ):
    ...
    def _perform_irc_calculation(
        ...
        if None not in [backward_irc, forward_irc]:
            ...
            write_output_file(
                backward_irc + 10 * (tsopt + ["\n"]) + forward_irc,
                os.path.join(output_dir, "irc_path.xyz"),
            )

            if None not in [backward_end, forward_end]:
                write_output_file(
                    backward_end + ["\n"] + tsopt + ["\n"] + forward_end,
                    os.path.join(output_dir, "reaction.xyz"),
                )
    ...
    def _finalize_reaction(
        ...
        try:
            pred_rc_smi_list = get_canonical_smiles_from_xyz_string(
                "".join(backward_end), self.charge
            )
            pred_pc_smi_list = get_canonical_smiles_from_xyz_string(
                "".join(forward_end), self.charge
            )
            ...
            if self.settings["check_reaction_smiles_end"]:
                ...
                if (
                    len(true_rc_smi_list) == len(pred_rc_smi_list)
                    and len(true_pc_smi_list) == len(pred_pc_smi_list)
                ) or (
                    len(true_rc_smi_list) == len(pred_pc_smi_list)
                    and len(true_pc_smi_list) == len(pred_rc_smi_list)
                ):
        ...
        if match:
            ...
            for file in ["tsopt.xyz", "reaction.xyz", "irc_path.xyz"]:
                shutil.copy2(os.path.join(output_dir, file), os.path.join(final_dir, file))
    ...
    def find_reaction_path(
        self, rc_conformer: Conformer, pc_conformer: Conformer, output_dir: str, final_dir: str
    ) -> bool:
        ...
        success = False
        if ts_found:
            irc_path_found, (backward_end, forward_end) = self._perform_irc_calculation(
                tsopt, output_dir
            )

            if irc_path_found:
                reaction_path_found = self._finalize_reaction(
                    backward_end, tsopt, forward_end, output_dir, final_dir
                )
