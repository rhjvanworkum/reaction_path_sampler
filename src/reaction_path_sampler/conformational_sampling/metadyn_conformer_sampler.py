...
from reaction_path_sampler.interfaces.xtb_utils import (
    compute_wall_radius,
    get_atom_constraints,
    get_fixing_constraints,
    get_metadynamics_settings,
    get_wall_constraint,
)
...
def optimize_autode_conformer(args):
    ...
    try:
        return Conformer(atoms=xyz_string_to_autode_atoms(opt_xyz), charge=charge, mult=mult)
    except Exception:
        return None
...
class MetadynConformerSampler(ConformerSampler):
    def __init__(self, smiles_strings: int, solvent: str, settings: dict[str, Any]) -> None:
        super().__init__(smiles_strings=smiles_strings, settings=settings, solvent=solvent)
    ...
    def sample_ts_conformers(
        ...
        self.settings["wall_radius"] = compute_wall_radius(mol=mol, settings=self.settings)
        ...
        t = time.time()
        confs = self._optimize_conformers(mol=mol, conformers=confs, fixed_atoms=fixed_atoms)
        print(f"optimizing conformers: {time.time() - t}")

        # 3. prune conformer set
        t = time.time()
        confs = self._prune_conformers(
            mol=mol, conformers=confs, use_graph_pruning=False, use_cregen_pruning=True, init="ts_"
        )
    ...
    def sample_conformers(
        ...
        if len(confs) < 10:
            confs = self._sample_metadynamics_conformers(
                mol=mol, post_fix="_tight", fixed_atoms=fixed_atoms
            )
            print(f"metadyn sampling: {time.time() - t}")
            print(f"metadyn sampled n conformers: {len(confs)}")
    ...
    def _sample_metadynamics_conformers(
        self, mol: MolecularSystem, post_fix: str = "", fixed_atoms: list[int] | None = None
    ) -> list[str]:
        """
        Sample conformers of a Molecule object complex.
        Returns a list of xyz strings containing conformers
        """
        xcontrol_settings = get_wall_constraint(self.settings["wall_radius"])
        xcontrol_settings += get_metadynamics_settings(
            mol, self.settings, len(self.smiles_strings), post_fix=post_fix
        )
        if fixed_atoms is not None:
            xcontrol_settings += get_atom_constraints(
                atom_idxs=fixed_atoms, force_constant=0.5, reference_file="ref.xyz"
            )
    ...
    def _optimize_conformers(
        self, mol: MolecularSystem, conformers: list[str], fixed_atoms: list[int] | None = None
    ) -> list[str]:
        """
        ...
        arguments = [
            (
                ...
                mol,
                self.solvent,
                self.settings["xtb_method"],
                xcontrol_settings,
                self.settings["xtb_n_cores"],
            )
        ...
        with ProcessPoolExecutor(max_workers=self.settings["n_processes"]) as executor:
            opt_conformers = list(
                tqdm(
                    executor.map(optimize_conformer, arguments),
                    total=len(arguments),
                    desc="Optimizing conformers",
...
if __name__ == "__main__":
    ...
    with open("/home/ruard/code/test_reactions/settings.yaml") as f:
