...
from reaction_path_sampler.graphs.lewis import mol_write_geom
...
def optimize_conformer_ff(args):
    ...
    new_coords = compute_ff_optimized_coords(atomic_symbols, coordinates, adj_mat, bond_mat)
...
class TopologyConformerSampler(ConformerSampler):
    def __init__(
        self, smiles_strings: list[str], settings: Any, solvent: str, mol: MolecularSystem
    ) -> None:
        super().__init__(smiles_strings=smiles_strings, settings=settings, solvent=solvent)
        self.mol = mol

        self.settings["wall_radius"] = compute_wall_radius(mol=self.mol, settings=self.settings)

    def sample_conformers(self, conformers: list[str]) -> list[str]:
        # 1. Find similar conformers
        t = time.time()
        confs = self._compute_ff_optimised_conformers(conformers=conformers)
        ...
        confs = self._prune_conformers(
            mol=self.mol, conformers=confs, use_graph_pruning=True, use_cregen_pruning=False
        )
    ...
    def _compute_ff_optimised_conformers(self, conformers: list[str]):
        arguments = []
        for conformer in conformers:
            ...
            arguments.append(
                (
                    atomic_symbols,
                    coordinates,
                    self.mol.connectivity_matrix,
                    self.mol.bond_order_matrix,
                )
            )

        with ProcessPoolExecutor(max_workers=self.settings["n_processes"]) as executor:
            pre_opt_conformers = list(
                tqdm(
                    executor.map(optimize_conformer_ff, arguments),
                    total=len(arguments),
                    desc="Optimizing conformers FF",
