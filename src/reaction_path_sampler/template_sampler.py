...
class TemplateSampler(ReactionSampler):
    ...
    def cartesian_coord_constraints(self) -> dict[int, np.ndarray]:
        if self._cartesian_coord_constraints is None:
            raise ValueError(
                "Cartesian Coord Constraints not set, call select_and_load_ts_template() first"
            )
        return self._cartesian_coord_constraints

    def select_and_load_ts_template(self, ts_templates: list[TStemplate]) -> None:
        for isomorphism_idx, (reactants, products) in enumerate(
            [(self.rc_complex, self.pc_complex), (self.pc_complex, self.rc_complex)]
        ):
            bond_rearrs = get_bond_rearrangs(products, reactants, name="test")
            if bond_rearrs is not None:
                for bond_rearr in bond_rearrs:
                    truncated_product_graph = get_truncated_active_mol_graph(
                        graph=products.graph, active_bonds=bond_rearr.all
                    )
                    ...
                    nx.set_node_attributes(
                        ...
                        {
                            node: products.coordinates[node]
                            for _, node in enumerate(truncated_product_graph.nodes)
                        },
                        "cartesian",
                    )
                    plot_networkx_mol_graph(truncated_product_graph)

                    for ts_template in ts_templates:
                        match, ignore_active_bonds = template_matches(
                            products, truncated_product_graph, ts_template
                        )
    ...
    def _constrained_optimization(
        ...
        opt_structure = xtb_driver(
            ...
            xcontrol_settings=get_fixing_constraints(
                [str(k + 1) for k in sorted(self.cartesian_coord_constraints.keys())]
            ),
            n_cores=2,
        )
    ...
    def optimize_ts_guess(self, ts_guess: Conformer, output_dir: str, final_dir: str) -> None:
