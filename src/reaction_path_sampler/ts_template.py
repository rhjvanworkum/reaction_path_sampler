...
from autode.transition_states.templates import (
    get_ts_template_folder_path,
    get_value_from_file,
    get_values_dict_from_file,
)

from reaction_path_sampler.graphs.xyz2mol import read_xyz_string
...
class TStemplate:
    ...
    def _save_to_file(self, file):
        ...
        title_line = f"TS template generated by autode v.{autode.__version__} on {date.today()}\n"
    ...
    def graph_has_correct_structure(self):
        ...
        for edge in self.graph.edges:
            ...
            if self.graph.edges[edge]["active"] and "distance" not in self.graph.edges[edge].keys():
    ...
    def load(self, filename):
        ...
        try:
            ...
            raise TemplateLoadingFailed("Failed to read file lines") from err
        ...
        for idx, data in nodes.items():
            if "cartesian_0" in data.keys():
                data["cartesian"] = np.array(
                    [data["cartesian_0"], data["cartesian_1"], data["cartesian_2"]]
                )
...
def save_ts_template(
    tsopt: str, complex: ade.Species, bond_rearr: BondRearrangement, output_dir: str
) -> TStemplate:
    ...
    truncated_graph = get_truncated_active_mol_graph(
        graph=complex.graph, active_bonds=bond_rearr.all
    )
    ...
    nx.set_node_attributes(
        truncated_graph,
        {node: complex.coordinates[node] for idx, node in enumerate(truncated_graph.nodes)},
        "cartesian",
    )
...
def get_constraints_from_template(
    ...
    mapping = get_mapping_ts_template(larger_graph=truncated_graph, smaller_graph=ts_template.graph)
