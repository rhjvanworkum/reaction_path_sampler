...
def construct_geometry_block(files: str | list[str], type: Literal["cart", "dlc"] = "cart") -> str:
    string = "geom:\n"
    string += f" type: {type}\n"
...
def construct_xtb_calculation_block(charge: int, mult: int, solvent: str | None = None) -> str:
    string = "calc:\n"
    string += " type: xtb\n"
...
def construct_orca_calculation_block(
    charge: int, mult: int, method: str = "B3LYP", basis: str = "6-31G", solvent: str | None = None
) -> str:
    string = "calc:\n"
    string += " type: orca\n"
...
def construct_tsopt_block() -> str:
    ...
    string += " type: rsirfo\n"
    string += " do_hess: True\n"
    string += " max_cycles: 75\n"
    string += " thresh: gau_tight\n"
    string += " hessian_recalc: 5\n"
    # string += f" geom:\n"
    # string += f"  type: cart\n"
    string += "\n"
    return string
...
def pysisyphus_driver(
    ...
    settings_string = construct_geometry_block(
        files=[file.split("/")[-1] for file in geometry_files], type="cart"
    )
    if method == "xtb":
        settings_string += construct_xtb_calculation_block(
            charge=charge, mult=mult, solvent=solvent
        )
    elif method == "orca":
    ...
    def execute_pysisyphus():
        ...
        cmd = "pysis settings.yaml"

        try:
            output = subprocess.check_output(cmd.split(), text=True, timeout=n_mins_timeout * 60)
        ...
        if job == "ts_opt":
            ...
            if os.path.exists("ts_opt.xyz"):
                with open("ts_opt.xyz") as f:
                    tsopt = f.readlines()
            else:
                if os.path.exists("ts_final_geometry.xyz"):
                    with open("ts_final_geometry.xyz") as f:
                        tsopt = f.readlines()
        ...
        if job == "ts_search":
            ...
            try:
                ...
                file = cycle_files[
                    max([int(file.split("_")[-1].split(".")[0]) for file in cycle_files])
                ]
            ...
            if os.path.exists("forward_end_opt.xyz"):
                with open("forward_end_opt.xyz") as f:
                    forward_end = f.readlines()
            if os.path.exists("backward_end_opt.xyz"):
                with open("backward_end_opt.xyz") as f:
...
if __name__ == "__main__":
    out = pysisyphus_driver(
        geometry_files="geodesic_path.xyz", charge=0, mult=0, jobs=["cos", "tsopt", "irc"]
    )
