...
def crest_driver(
    ...
    def execute_crest():
        ...
        # Default to an empty ensemble: if CREST fails to produce its output
        # file we previously raised UnboundLocalError on the return below.
        structures: list[str] = []
        if os.path.exists("crest_ensemble.xyz"):
            structures, _energies = read_trajectory_file("crest_ensemble.xyz")
